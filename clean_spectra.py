...
def download_data(base_directory: str, configs: str):
    ...
    if os.path.isdir(os.path.join(base_directory, 'raw_data')):
        pass
    else:
        os.mkdir(os.path.join(output_directory, 'raw_data'))
    ...
    for data_set in datasets:
        ...
        # create directories to download data
        if os.path.isdir(os.path.join(base_directory, 'raw_data', data_set)):
            pass
        else:
            os.mkdir(os.path.join(base_directory, 'raw_data', data_set))
        ...
        if url == 'UNK':
            ...
            download_location = os.path.join(base_directory, 'raw_data', data_set, local_filename)
            ...
            #TODO - add function to unzip file and keep only the .csv tables
...
class clean:
    ...
    def convolve(self, wavelength_file: str, geo_filter: bool, level: str):
        ...
        print("loading instrument resolution...", wavelength_file)

        wvls, fwhms = load_wavelengths(self.wavelength_file)
        ...
        for i in tables:
            ...
            for index, row in tqdm(df.iterrows(), ncols=100, total=df.shape[0], ascii=True, desc=msg_progess_bar):
                ...
                pool.apply_async(parallel_convole, args=(em_spectra, em_wvls, wvls, fwhms, classifications),
